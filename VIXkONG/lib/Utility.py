...
def getData(start, end):
    ...
    spx = yf.download (tickers = "^SPX", start = start,
                                end = end, interval = "1d").reset_index()
    data = yf.download (tickers = "^VIX", start = start,
                                end = end, interval = "1d").reset_index()
    ...
    data['feat_spxGap'] = (spx['Open']-spx['Close'].shift(1))/spx['Close'].shift(1)
    data['feat_spxPrevRange'] = (spx['High'].shift(1)-spx['Low'].shift(1))/spx['Open'].shift(1)
    data['feat_Spx_Open-PrevClose'] = np.where(spx['Open']-spx['Close'].shift(1)>0,1,0)
    data['feat_Open-PrevClose'] = np.where(data['Open']-data['Close'].shift(1)>0,1,0)
    data['feat_vixOpen'] = data['Open']
    data['feat_vixPrevClose'] = data['Close'].shift(1)
    ...
    spx = spx.drop(['Open','Close', 'High','Low', 'Adj Close', 'Volume'], axis=1)
    vvix = vvix.drop(['Open','Close', 'High','Low', 'Adj Close', 'Volume'], axis=1)
    data = data.merge(spx, how='left', on='Date')# VIX
